...
{
   if( g_currESC == 0 ){
     PORTB = B00000010; // Reset pin 0 and set pin 1
     //ESC0.reset();
     //ESC1.set();
     *OCRnA = ESC1.GetTicks();
  }else if( g_currESC == 1 ){
     PORTB = B00000100; // Reset pin 1 and set pin 0
     //ESC1.reset();
     //ESC2.set();
     *OCRnA = ESC2.GetTicks();
  }else if( g_currESC == 2 ){
     PORTB = B00001000; // Reset pin 2 and set pin 1
     //ESC2.reset();
     //ESC3.set();
     *OCRnA = ESC3.GetTicks();
  }else{
    PORTB = B00000001; // Reset pin 3 and set pin 0
  //  ESC3.reset();
    g_currESC = -1;
  //  ESC0.set();
    *OCRnA = ESC0.GetTicks();
