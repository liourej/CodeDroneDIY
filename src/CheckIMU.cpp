...
bool CheckGyro(MPU6050 _accelgyro)
{
    ...
    rawData[0] =
            _accelgyro
                    .getAccelXSelfTestFactoryTrim(); // X-axis self-test results
    rawData[1] =
            _accelgyro
                    .getAccelYSelfTestFactoryTrim(); // Y-axis self-test results
    rawData[2] =
            _accelgyro
                    .getAccelZSelfTestFactoryTrim(); // Z-axis self-test results
    ...
    for (int axis = 3; axis < 6; axis++) { // Gyro data
        if (axis == 4) // If axis is Y, computation is different (negative sign)
            ...
            FT[axis] =
                    ComputeGyroFactoryTrimValue(dataTestEnabled[axis], false);
    ...
    I2Cdev::writeBit(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_CONFIG,
                     MPU6050_GCONFIG_XG_ST_BIT, false);
    I2Cdev::writeBit(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_CONFIG,
                     MPU6050_GCONFIG_XG_ST_BIT, false);
    I2Cdev::writeBit(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_GYRO_CONFIG,
                     MPU6050_GCONFIG_XG_ST_BIT, false);
